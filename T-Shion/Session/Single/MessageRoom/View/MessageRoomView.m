...
@interface MessageRoomView () <UITableViewDelegate,UITableViewDataSource,AVAudioPlayerDelegate,YMImageBrowserDataSource>
@property (weak, nonatomic) MessageRoomViewModel *viewModel;
...
@property (strong, nonatomic) NSMutableArray *displayArray;//显示的数据
...
@implementation MessageRoomView

- (instancetype)initWithViewModel:(id<BaseViewModelProtocol>)viewModel {
    self.viewModel = (MessageRoomViewModel*)viewModel;
...
- (void)bindViewModel {
    @weakify(self)
    [[self.viewModel.refreshMsgListSubject takeUntil:self.rac_willDeallocSignal] subscribeNext:^(id _Nullable x) {
        @strongify(self)
...
- (void)scrollToFirstUnreadMsg {
    ...
    [FMDBManager ChangeAllMessageReadStatusWithRoomId:self.viewModel.friendModel.roomId];
}
...
- (void)scrollToBottomAnimated:(BOOL)animated {
    ...
    if (count >0) {
        if (animated) {
            ...
            if (offsetHeight>self.table.height) {
                return;
            }
        }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(nonnull NSIndexPath *)indexPath {
    ...
    if (model.msgType == MESSAGE_NotifyTime) {
        ...
        return model.contentHeight + 10 + 15;//10高度是顶部距离+15底部距离
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (message.senderInfo == nil) {
        message.senderInfo = self.viewModel.friendModel;
    }
    ...
    if(!cell) {
        ...
        if (message.msgType == MESSAGE_AUDIO) {
            ...
            MessageRTCView *textView = (MessageRTCView *)cell.bubbleView;
            textView.rtcCallBlock = ^(MessageModel * _Nonnull model) {
                @strongify(self)
                [self.viewModel.rtcCallSubject sendNext:model];
            };
    ...
    cell.tag = tag;
    if (message.messageId.integerValue == 1000) {
...
//        popoverView.showShade = YES;
        ...
        if (model.msgType !=  MESSAGE_RTC && !self.viewModel.isCrypt) {
        ...
        if (model.msgType == MESSAGE_TEXT && !self.viewModel.isCrypt) {
...
- (void)deleteMessage:(id)delete {
    ...
    if (result) {
        ...
        [self.viewModel.refreshMsgListSubject sendNext:@(REFRESH_Table_MESSAGES)];
    }
}
...
- (MessageRoomToolView *)toolView {
    if (!_toolView) {
        ...
        _toolView.uid = self.viewModel.friendModel.userId;
        //modify by chw 2019.04.22 for "加密房间和普通房间的区分，草稿会错乱"
        NSString *roomId = self.viewModel.isCrypt? self.viewModel.friendModel.encryptRoomID : self.viewModel.friendModel.roomId;
        _toolView.toolBar.singleChatRoomID = roomId;
        _toolView.folderPath = [[TShionSingleCase doucumentPath] stringByAppendingPathComponent:roomId];
