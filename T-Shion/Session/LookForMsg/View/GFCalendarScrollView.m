...
@property (nonatomic, assign) BOOL displayToday;//显示是否是今天的的月份
@end
...
//    [self addSubview:_collectionViewL];
    ...
    _collectionViewM = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, selfWidth, selfHeight) collectionViewLayout:flowLayout];
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    GFCalendarMonth *calendar = self.monthArray[1];
    NSInteger total = calendar.totalDays +calendar.firstWeekday;
    int number = total % 7;
    if (number == 0) {
        return total;
    }else {
        NSInteger count = total + (7 - number);
        return count; // 7 * 6
    }
}
...
//                    cell.todayCircle.backgroundColor = RGB(84, 208, 172);
...
//    } else
    if (collectionView == _collectionViewM) {
        ...
        if (indexPath.row >= firstWeekday && indexPath.row < firstWeekday + totalDays) {
            cell.todayLabel.text = [NSString stringWithFormat:@"%ld", indexPath.row - firstWeekday + 1];
            cell.todayLabel.textColor = [UIColor darkGrayColor];
            cell.todayLabel.textColor = [UIColor colorWithRed:29/255 green:29/255 blue:38/255 alpha:0.5];
...
- (void)reloadCollectionDataWithType:(int)type {
    if (type == 0) {
        self.displayToday = NO;
...
- (void)performAnimations:(NSString *)transition{
    ...
    [self.collectionViewM.layer addAnimation:catransition forKey:nil];
}
@end
