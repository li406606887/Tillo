...
- (void)connectToHost {
    ...
    // 获取分配的 主机ip 和 端口号
//    NSString *hostAddress = @"192.168.1.124:3000";
...
/**
 发送消息
 */
- (void)sendPushMessageWithContent:(NSMutableDictionary *)contentDic apiUrl:(NSString *)api{
    // 通过http代理发送数据
    MPHttpRequest *request = [MPHttpRequest POST:api andParams:contentDic];
