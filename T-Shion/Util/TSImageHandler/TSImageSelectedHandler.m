...
- (NSArray *)selectedAssets {
    ...
    dispatch_sync(self.concurrentQueue, ^{
        assets = [NSArray arrayWithArray:self->_dataArray];
    });
    return assets;
}
...
- (NSArray *)selectedIndexs {
    ...
    dispatch_sync(self.concurrentQueue, ^{
        indexs = [NSArray arrayWithArray:self->_indexArray];
    });
    return indexs;
}
...
- (void)addIndex:(id)index {
    ...
    dispatch_barrier_async(self.concurrentQueue, ^{
        [self->_indexArray addObject:index];
    });
}
...
- (void)removeIndex:(id)index {
    ...
    dispatch_barrier_async(self.concurrentQueue, ^{
        [self->_indexArray removeObject:index];
    });
}

- (void)removeAllIndexs {
    dispatch_barrier_async(self.concurrentQueue, ^{
        [self->_indexArray removeAllObjects];
    });
}
...
- (void)addAsset:(id)asset {
    if (!asset) {return;}
    dispatch_barrier_async(self.concurrentQueue, ^{
        [self->_dataArray addObject:asset];
    });
}
...
- (void)removeAsset:(id)asset {
    ...
    dispatch_barrier_async(self.concurrentQueue, ^{
        [self->_dataArray removeObject:asset];
    });
}
