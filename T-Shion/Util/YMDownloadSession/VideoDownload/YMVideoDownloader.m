...
@interface YMVideoDownloader()<NSURLSessionDelegate>
{
...
@implementation YMVideoDownloader

- (instancetype)init {
    if (self = [super init]) {
        NSLog(@"[YMVideoDownloader init]");
...
+ (instancetype)downloader {
    ...
    static YMVideoDownloader *_downloader;
...
- (NSString *)backgroundSessionIdentifier {
    ...
    NSString *identifier = [NSString stringWithFormat:@"%@.BGS.YMVideoDownloader", bundleId];
    return identifier;
}
...
- (void)recoveryExceptionTasks {
    ...
    [dictM.copy enumerateKeysAndObjectsUsingBlock:^(NSNumber *_Nonnull key, NSURLSessionDownloadTask *obj, BOOL * _Nonnull stop) {
        YMDownloadTask *task = [YMVideoDownloadDB taskWithStid:key.integerValue].firstObject;
...
- (YMDownloadTask *)resumeDownloadTaskWithTid:(NSString *)tid progress:(YMProgressHandler)progress completion:(YMCompletionHandler)completion {
    YMDownloadTask *task = [YMVideoDownloadDB taskWithTid:tid];
...
- (void)prepareRecreateSession {
    ...
    [[YMVideoDownloadDB fetchAllDownloadTasks] enumerateObjectsUsingBlock:^(YMDownloadTask * _Nonnull task, NSUInteger idx, BOOL * _Nonnull stop) {
        if (task.downloadTask && task.downloadTask.state == NSURLSessionTaskStateRunning) {
...
//    NSArray *taskArray = [YMVideoDownloadDB itemsWithUrl:task.downloadURL uid:ta]
//    if (task) {
//        YMDownloadItem *item = [YMVideoDownloadDB itemWithTaskId:task.taskId];
//        if (!item) {
...
- (void)saveDownloadTask:(YMDownloadTask *)task {
    [YMVideoDownloadDB saveTask:task];
}
...
- (YMDownloadTask *)taskWithSessionTask:(NSURLSessionDownloadTask *)downloadTask {
    ...
    if (!task) {
        NSArray <YMDownloadTask *>* tasks = [YMVideoDownloadDB taskWithStid:[self sessionTaskIdWithDownloadTask:downloadTask]];
    ...
    if (!task) {
        NSArray *tasks = [YMVideoDownloadDB taskWithUrl:url];
...
- (void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didFinishDownloadingToURL:(NSURL *)location {
    ...
    if (fileSize>0 && fileSize != task.fileSize) {
        NSString *errStr = [NSString stringWithFormat:@"[YMVideoDownloader didFinishDownloadingToURL] fileSize Error, task fileSize: %lld tmp fileSize: %lld", task.fileSize, fileSize];
        NSLog(@"%@",errStr);
