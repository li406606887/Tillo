...
#define kYMDownloadVersionKey @"YMDownloadVersionKey"
#define kYMDownloadDbMinUpgradeVerion @"1.0.0" //数据库的表和字段更新需要改变版本号

#if YMDownload_Mgr_Item
@interface YMDownloadItem (YMImageDownloadDB)
...
@interface YMDownloadTask (YMImageDownloadDB)
@property (nonatomic, assign) NSInteger pid;
@property (nonatomic, assign, readonly) NSUInteger createTime;
+ (instancetype)taskWithDict:(NSDictionary *)dict;
@end

typedef NS_ENUM(NSUInteger, YMDownloadDBValueType) {
    YMDownloadDBValueTypeNull,
    YMDownloadDBValueTypeString,
    YMDownloadDBValueTypeNumber,
    YMDownloadDBValueTypeData
};

@implementation YMImageDownloadDB
...
static NSMutableDictionary <NSString* ,YMDownloadTask *> *_memCacheTasks;
...
static NSMutableDictionary <NSString* ,YMDownloadItem *> *_memCacheItems;
#endif
...
+ (void)initDatabase {
    _dbQueue = dispatch_queue_create("YMImageDownloadDB_Queue", DISPATCH_QUEUE_SERIAL);
    
    //doc路径
    NSString *path = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, true).firstObject;
    
    path = [path stringByAppendingPathComponent:@"YMImageDownload"];
    
    [YMDownloadUtils ym_createPathIfNotExist:path];
    
    path = [path stringByAppendingPathComponent:@"YMImageDownload.db"];
    ...
    BOOL performBlock = [self performBlock:^BOOL{
        return [self execSql:sql];
    } sync:YES];
    
    if (performBlock) {
        NSLog(@"[init db success]");
    }
...
+ (void)compatibleDatabase {
    NSString *localVersion = [[NSUserDefaults standardUserDefaults] valueForKey:kYMDownloadVersionKey];
    NSString *curVersion = [YMDownloadTask downloaderVerison];
    
    if ([curVersion compare:localVersion options:NSNumericSearch] == NSOrderedDescending){
        [[NSUserDefaults standardUserDefaults] setValue:curVersion forKey:kYMDownloadVersionKey];
        if ([kYMDownloadDbMinUpgradeVerion compare:localVersion options:NSNumericSearch] == NSOrderedDescending) {
...
+ (YMDownloadDBValueType)getTypeWithValue:(id)value {
    YMDownloadDBValueType type = YMDownloadDBValueTypeNull;
    if ([value isKindOfClass:[NSString class]]) {
        type = YMDownloadDBValueTypeString;
    }else if ([value isKindOfClass:[NSNumber class]]){
        type = YMDownloadDBValueTypeNumber;
    }else if ([value isKindOfClass:[NSData class]]){
        type = YMDownloadDBValueTypeData;
    }
    return type;
}
...
+ (void)getSqlWithKeys:(const char **)keys
                 count:(int)count
                   obj:(id)obj
               oldItem:(NSDictionary *)oldItem
        enumerateBlock:(void (^)(YMDownloadDBValueType type, NSString *key, id value,  int idx))enumerateBlock {
...
+ (void)saveAllData {
    [self performBlock:^BOOL{
        //fixme: transaction
        [_memCacheTasks enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, YMDownloadTask * _Nonnull obj, BOOL * _Nonnull stop) {
            [self saveDownloadTask:obj];
        }];
#if YMDownload_Mgr_Item
        [_memCacheItems enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, YMDownloadItem * _Nonnull obj, BOOL * _Nonnull stop) {
...
+ (YMDownloadItem *)itemWithDict:(NSDictionary *)dict {
    ...
    if (!item) {
        item = [YMDownloadItem itemWithDict:dict];
...
+ (NSArray <YMDownloadItem *> *)fetchAllDownloadItemWithUid:(NSString *)uid {
    ...
    [self performBlock:^BOOL{
        ...
        [rel enumerateObjectsUsingBlock:^(NSDictionary * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            YMDownloadItem *item = [self itemWithDict:obj];
...
+ (NSArray <YMDownloadItem *> *)fetchAllDownloadedItemWithUid:(NSString *)uid {
    __block NSMutableArray *results = [NSMutableArray array];
    [self performBlock:^BOOL{
        NSString *sql = [NSString stringWithFormat:@"select * from downloadItem where downloadStatus == %lu and uid == '%@' ORDER BY createTime", (unsigned long)YMDownloadStatusFinished, uid];
        NSArray *rel = [self selectSql:sql];
        [rel enumerateObjectsUsingBlock:^(NSDictionary * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            YMDownloadItem *item = [self itemWithDict:obj];
            if(item) [results addObject:item];
...
+ (YMDownloadItem *)itemWithTaskId:(NSString *)taskId {
    __block YMDownloadItem *item = _memCacheItems[taskId];
    if(!item){
        [self performBlock:^BOOL{
...
+ (NSArray *)itemsWithUrl:(NSString *)downloadUrl uid:(NSString *)uid{
    ...
    [self performBlock:^BOOL{
        ...
        [rel enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            YMDownloadItem *item = [self itemWithDict:rel.firstObject];
...
+ (YMDownloadItem *)itemWithFid:(NSString *)fid uid:(NSString *)uid{
    __block YMDownloadItem *item = nil;
    [self performBlock:^BOOL{
...
+ (BOOL)updateItemExtraData:(YMDownloadItem *)item {
    BOOL result = false;
...
+ (BOOL)updateItem:(YMDownloadItem *)item withResults:(NSArray *)results {
    NSMutableString *updateSql = [NSMutableString string];
    int count = sizeof(allItemKeys) / sizeof(allItemKeys[0]);
    [self getSqlWithKeys:allItemKeys count:count obj:item oldItem:results.firstObject enumerateBlock:^(YMDownloadDBValueType type, NSString *key, id value, int idx) {
        if (type == YMDownloadDBValueTypeData) {
            [self updateItemExtraData:item];
        }else if (type == YMDownloadDBValueTypeNumber){
            [updateSql appendFormat:@"%@%@=%@", updateSql.length !=0 ? @", " : @"", key, [value stringValue]];
        }else if(type == YMDownloadDBValueTypeNull){
...
+ (BOOL)saveDownloadItem:(YMDownloadItem *)item {
    ...
    if (results.count==0) {
        ...
        int count = sizeof(allItemKeys) / sizeof(allItemKeys[0]);
        [self getSqlWithKeys:allItemKeys count:count obj:item oldItem:nil enumerateBlock:^(YMDownloadDBValueType type, NSString *key, id value, int idx) {
            if (type == YMDownloadDBValueTypeNumber){
            ...
            }else if(type == YMDownloadDBValueTypeString){
                [insertSqlKeys appendFormat:@"%@%@", insertSqlKeys.length!=0 ? @", ": @"", key];
...
#pragma mark - task
+ (YMDownloadTask *)taskWithDict:(NSDictionary *)dict {
    if(!dict) return nil;
    NSString *taskId = [dict valueForKey:@"taskId"];
    NSAssert(taskId, @"taskId can not nil!");
    if(!taskId) return nil;
    YMDownloadTask *task = [_memCacheTasks valueForKey:taskId];
    if (task && ![task.taskId isEqualToString:taskId]) {
        _memCacheTasks[taskId] = nil;
        return nil;
    }
    if (!task) {
        task = [YMDownloadTask taskWithDict:dict];
...
+ (YMDownloadTask *)taskWithTid:(NSString *)tid {
    if(!tid) return nil;
    __block YMDownloadTask *task = [_memCacheTasks valueForKey:tid];
    if(!task){
        [self performBlock:^BOOL{
...
+ (NSArray <YMDownloadTask *> *)taskWithUrl:(NSString *)url {
    ...
    [self performBlock:^BOOL{
        ...
        NSArray *rel = [self selectSql:sql];
        [rel enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            YMDownloadTask *task = [self taskWithDict:obj];
...
+ (NSArray *)taskWithStid:(NSInteger)stid {
    ...
    [self performBlock:^BOOL{
        ...
        NSArray *rel = [self selectSql:sql];
        [rel enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            YMDownloadTask *task = [self taskWithDict:rel.firstObject];
            [tasks addObject:task];
        }];
        return true;
    } sync:true];
...
+ (BOOL)removeTask:(YMDownloadTask *)task {
    ...
    return [self performBlock:^BOOL{
        ...
        NSLog(@"任务删除--%@", task.taskId);
        return result;
    } sync:false];
}

+ (BOOL)updateTask:(YMDownloadTask *)task withResults:(NSArray *)results {
    NSMutableString *updateSql = [NSMutableString string];
    int count = sizeof(allTaskKeys) /sizeof(allTaskKeys[0]);
    [self getSqlWithKeys:allTaskKeys count:count obj:task oldItem:results.firstObject enumerateBlock:^(YMDownloadDBValueType type, NSString *key, id value, int idx) {
        if (type == YMDownloadDBValueTypeData) {
...
+ (BOOL)saveDownloadTask:(YMDownloadTask *)task {
    if (!task) {
        return NO;
    }
    ...
    if (results.count==0) {
        ...
        NSMutableString *insertSqlValues = [NSMutableString string];
        int count = sizeof(allTaskKeys) / sizeof(allTaskKeys[0]);
        [self getSqlWithKeys:allTaskKeys count:count obj:task oldItem:nil enumerateBlock:^(YMDownloadDBValueType type, NSString *key, id value, int idx) {
            if (type == YMDownloadDBValueTypeNumber){
                [insertSqlKeys appendFormat:@"%@%@", insertSqlKeys.length!=0 ? @", ": @"", key];
                [insertSqlValues appendFormat:@"%@%@", insertSqlValues.length!=0 ? @", " : @"", [value stringValue]];
            }else if(type == YMDownloadDBValueTypeString){
