...
@interface YMDownloader()<NSURLSessionDelegate>
{
...
+ (instancetype)downloader {
    ...
    static YMDownloader *_downloader;
...
- (NSString *)backgroundSessionIdentifier {
    ...
    NSString *identifier = [NSString stringWithFormat:@"%@.BGS.YMDownloader", bundleId];
    return identifier;
}
...
- (void)recoveryExceptionTasks {
    ...
    [dictM.copy enumerateKeysAndObjectsUsingBlock:^(NSNumber *_Nonnull key, NSURLSessionDownloadTask *obj, BOOL * _Nonnull stop) {
        YMDownloadTask *task = [YMDownloadDB taskWithStid:key.integerValue].firstObject;
...
- (YMDownloadTask *)resumeDownloadTaskWithTid:(NSString *)tid progress:(YMProgressHandler)progress completion:(YMCompletionHandler)completion {
    YMDownloadTask *task = [YMDownloadDB taskWithTid:tid];
...
- (void)prepareRecreateSession {
    ...
    [[YMDownloadDB fetchAllDownloadTasks] enumerateObjectsUsingBlock:^(YMDownloadTask * _Nonnull task, NSUInteger idx, BOOL * _Nonnull stop) {
        if (task.downloadTask && task.downloadTask.state == NSURLSessionTaskStateRunning) {
...
- (YMDownloadTask *)taskWithSessionTask:(NSURLSessionDownloadTask *)downloadTask {
    ...
    if (!task) {
        NSArray <YMDownloadTask *>* tasks = [YMDownloadDB taskWithStid:[self sessionTaskIdWithDownloadTask:downloadTask]];
    ...
    if (!task) {
        NSArray *tasks = [YMDownloadDB taskWithUrl:url];
...
- (void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didFinishDownloadingToURL:(NSURL *)location {
    ...
    if (fileSize>0 && fileSize != task.fileSize) {
        NSString *errStr = [NSString stringWithFormat:@"[YMDownloader didFinishDownloadingToURL] fileSize Error, task fileSize: %lld tmp fileSize: %lld", task.fileSize, fileSize];
        NSLog(@"%@",errStr);
