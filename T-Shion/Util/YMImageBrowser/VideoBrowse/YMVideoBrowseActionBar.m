...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        [self setThumbImage:[UIImage imageNamed:@"ymib_circlePoint@3x"] forState:UIControlStateNormal];
...
@interface YMVideoBrowseActionBar () {
    BOOL _isDragging;
}
...
@property (nonatomic, strong) YMVideoBrowseActionSlider *slider;

@end


@implementation YMVideoBrowseActionBar
...
- (CGRect)getFrameWithContainerSize:(CGSize)containerSize {
    CGFloat height = kActionBarDefaultsHeight + YMIB_HEIGHT_EXTRABOTTOM;
...
- (void)clickPlayButton:(UIButton *)button {
    ...
    if (button.selected) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(ym_videoBrowseActionBar:clickPauseButton:)]) {
            [self.delegate ym_videoBrowseActionBar:self clickPauseButton:button];
        }
    } else {
        if (self.delegate && [self.delegate respondsToSelector:@selector(ym_videoBrowseActionBar:clickPlayButton:)]) {
            [self.delegate ym_videoBrowseActionBar:self clickPlayButton:button];
        }
    }
...
- (void)respondsToSliderTouchFinished:(UISlider *)slider {
    if (self.delegate && [self.delegate respondsToSelector:@selector(ym_videoBrowseActionBar:changeValue:)]) {
        [self.delegate ym_videoBrowseActionBar:self changeValue:slider.value];
    }
}
...
- (UIButton *)playButton {
    if (!_playButton) {
        ...
        [_playButton setImage:[UIImage imageNamed:@"ymib_play"] forState:UIControlStateNormal];
        [_playButton setImage:[UIImage imageNamed:@"ymib_pause"] forState:UIControlStateSelected];
