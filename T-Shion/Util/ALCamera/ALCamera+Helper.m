...
static NSString *kVideoFold = @"Video";

@implementation ALCamera (Helper)
...
- (NSString *)createDefaultVideoFolderPath
{
    ...
    if (homePath.length > 0)
    {
        ...
        if ([fileManager fileExistsAtPath:documentPath isDirectory:NULL] == YES)
        {
            ...
            //offline file folder
            tmpFilePath = [documentPath stringByAppendingString:[NSString stringWithFormat:@"/%@", kVideoFold]];
            if ([paths containsObject:kVideoFold] == NO)
            {
...
- (NSString *)createVideoFilePath {
    ...
    if (self.videoFoldPath) {
        foldPath = self.videoFoldPath;
    } else {
        foldPath = [self createDefaultVideoFolderPath];
    }
    
    self.videoFileName = [self createVideoFileName];
    
    NSString *finalPath = [NSString stringWithFormat:@"%@/%@", foldPath, self.videoFileName];
